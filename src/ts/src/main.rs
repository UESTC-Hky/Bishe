...
async fn main() -> Result<()> {
    ...
    let ls_client = client::LsClient::new(ls_addr)
        .await
        .expect("TS: 连接LS服务失败");

    // 初始化 SS 客户端
    let ss_addr = format!("http://{}", args.ss_addr);
    let ss_client = client::SsClient::new(ss_addr)
        .await
        .expect("TS: 连接SS服务失败");
    ...
    // 创建事务管理器（支持ACID），如果WAL损坏则自动清理后重试
    let tx_manager = match TransactionManager::new(ls_client, ss_client, wal_dir.clone()).await {
        Ok(mgr) => mgr,
        Err(e) => {
            tracing::warn!("创建事务管理器失败: {}, 清理损坏的WAL并重试", e);
            // 删除损坏的WAL文件，让服务从头开始
            let _ = std::fs::remove_dir_all(&wal_dir);
            let _ = std::fs::create_dir_all(&wal_dir);

            // 重新连接LS和SS客户端
            let ls_addr2 = format!("http://{}", args.ls_addr);
            let ss_addr2 = format!("http://{}", args.ss_addr);
            let ls_client2 = client::LsClient::new(ls_addr2)
                .await
                .expect("TS: 重连LS服务失败");
            let ss_client2 = client::SsClient::new(ss_addr2)
                .await
                .expect("TS: 重连SS服务失败");

            TransactionManager::new(ls_client2, ss_client2, wal_dir)
                .await
                .expect("TS: 清理WAL后仍无法创建事务管理器")
        }
    };
